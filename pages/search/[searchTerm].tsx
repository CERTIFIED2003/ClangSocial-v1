...
const Search = ({ videos }: { videos: Video[] }) => {
  ...
  return (
    <div className="w-full">
      <div className="flex gap-10 mb-10 mt-7 border-b-2 border-gray-200 bg-white w-full">
        <p
            ...
            searchedAccounts.map((user: IUser, idx: number) => (
              <Link key={idx} href={`/profile/${user._id}`}>
                <div className="flex gap-3 p-2 cursor-pointer font-semibold rounded border-b-2 border-gray-200">
                  ...
                  <div>
                    <div>
                      <p className='flex gap-1 items-center text-lg font-semibold text-primary'>
        ...
        <div className='md:mt-16 flex flex-wrap gap-6 md:justify-start '>
          {videos.length ? (
            videos.map((post: Video, idx: number) => (
              <VideoCardSearch post={post} key={post.previewtype} />
            ))
          ) : (
